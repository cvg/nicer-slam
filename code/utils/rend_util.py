...
def get_ssim(img1, img2, res, ssim_computer):
    img1 = img1.reshape(1, res[0], res[1], 3).permute(0, 3, 1, 2)
    img2 = img2.reshape(1, res[0], res[1], 3).permute(0, 3, 1, 2)
    ssim = ssim_computer(img1, img2)
    return ssim


def get_lpips(img1, img2, res, lpips_computer):
    img1 = img1.reshape(1, res[0], res[1], 3).permute(0, 3, 1, 2)
    img2 = img2.reshape(1, res[0], res[1], 3).permute(0, 3, 1, 2)
    lpips_val = lpips_computer(img1, img2)
    return lpips_val
...
def get_camera_params(uv, pose, intrinsics):
    ...
    ray_dirs = ray_dirs / (ray_dirs * ray_dirs).sum(-1, keepdim=True)
