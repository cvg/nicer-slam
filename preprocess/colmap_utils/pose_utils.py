...
def load_colmap_data(realdir, return_intrinsics=False, pycolmap_output=False):
    if pycolmap_output:
        subfolder='output'
    else:
        subfolder='sparse/0'
    camerasfile = os.path.join(realdir, f'{subfolder}/cameras.bin')
    ...
    imagesfile = os.path.join(realdir, f'{subfolder}/images.bin')
    ...
    points3dfile = os.path.join(realdir, f'{subfolder}/points3D.bin')
    pts3d = read_model.read_points3d_binary(points3dfile)
    
    # must switch to [-u, r, -t] from [r, -u, t], NOT [r, u, -t]
    # poses = np.concatenate([poses[:, 1:2, :], poses[:, 0:1, :], -poses[:, 2:3, :], poses[:, 3:4, :], poses[:, 4:5, :]], 1)
    if return_intrinsics:
        return poses, cam.params, pts3d, perm
    else:
